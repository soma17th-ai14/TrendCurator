...
def _build_where(
    ...
    # ChromaDB $gte/$lte는 숫자만 지원 — published_at_int(YYYYMMDD 정수)로 비교
    conditions = []
    if date_from:
        conditions.append({"published_at_int": {"$gte": int(date_from.strftime("%Y%m%d"))}})
    if date_to:
        conditions.append({"published_at_int": {"$lte": int(date_to.strftime("%Y%m%d"))}})
    if sources:
