...
공개 API 경로는 `/api/v1` 아래에 마운트됩니다. UI 가 호출하는 질의 엔드포인트는 다음 흐름을 따릅니다.
...
- `app/api`: FastAPI 외부 API 라우터.
- `app/graphs`: LangGraph 워크플로우 orchestration.
- `frontend`: `/api/v1` API 를 호출하는 Streamlit UI.

LangGraph 내부에서는 에이전트 계약의 intent 라벨을 그대로 사용합니다.

- `GENERAL_QA`
- `TREND_COMPARISON`

외부 API 응답에서는 위 라벨을 다음 값으로 매핑합니다.
