...
공개 API 라우트는 `/api/v1` 아래에 마운트합니다. UI에서 사용하는 질의 엔드포인트는 다음 흐름으로 동작합니다.
...
- `app/graphs`: LangGraph 워크플로우 오케스트레이션
- `frontend`: `/api/v1` API를 호출하는 Streamlit UI

그래프 내부에서는 에이전트 계약의 intent label을 그대로 사용합니다.
